...
const Modal = ({ onClose, toggle }) => {
  return createPortal(
    <div className="fixed inset-0 bg-background/60 backdrop-blur-sm flex items-center justify-center z-50">
      ...
      py-8 px-6 xs:px-10 sm:px-16 mx-8 rounded shadow-glass-inset text-center space-y-8"
      >
  ...
  return (
    <div className="fixed top-4 right-2.5 xs:right-4 z-50 group">
      ...
      <motion.button
        ...
        animate={{ scale: 1 }}
        transition={{ delay: 1 }}
        className="w-10 h-10 xs:w-14 xs:h-14 text-foreground rounded-full flex items-center justify-center z-50 cursor-pointer p-2.5 xs:p-4 custom-bg"
        aria-label={"home"}
        name={"home"}
      >
        {isPlaying ? (
